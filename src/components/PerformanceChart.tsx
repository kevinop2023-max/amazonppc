    ...
    <div className="bg-white border border-gray-100 rounded-xl shadow-lg p-3 text-xs space-y-1 min-w-[200px]">
      {(row?.bDate || row?.aDate) && (
        <div className="pb-1.5 mb-0.5 border-b border-gray-50 space-x-2">
          {row.bDate && <span className="text-purple-600 font-medium">B: {fmtDate(row.bDate)}</span>}
          {row.aDate && <span className="text-blue-500 font-medium">A: {fmtDate(row.aDate)}</span>}
        </div>
      )}
      {payload.map((p: any) => {
...
export default function PerformanceChart({ dataA, dataB, labelA, labelB }: Props) {
  ...
  const pills = [
    { key: 'clicks', label: 'Clicks',     bVal: totB.clicks.toLocaleString() },
    { key: 'spend',  label: 'Total Cost', bVal: `$${(totB.spendCents / 100).toFixed(2)}` },
    { key: 'orders', label: 'Orders',     bVal: totB.orders.toLocaleString() },
    { key: 'sales',  label: 'Sales',      bVal: `$${(totB.salesCents / 100).toFixed(2)}` },
  ]
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          {pills.map(m => {
            ...
            return (
              <button key={m.key} onClick={() => toggle(m.key)} className="flex items-center gap-2 text-left">
                ...
                />
                <div>
                  <div className={`text-[11px] font-medium leading-none mb-0.5 transition-colors ${isOn ? 'text-gray-500' : 'text-gray-300'}`}>
                    {m.label}
                  </div>
      ...
      <ResponsiveContainer width="100%" height={220}>
        <ComposedChart data={rows} margin={{ top: 4, right: hasRight ? 52 : 8, bottom: 0, left: 4 }}>
          ...
          {active.spend && <>
            <Line yAxisId="right" dataKey="aSpend"  name="Cost (A)"   stroke={METRICS[1].aColor} strokeWidth={1.5} strokeDasharray="4 2" dot={false} connectNulls />
            <Line yAxisId="right" dataKey="bSpend"  name="Cost (B)"   stroke={METRICS[1].bColor} strokeWidth={2}   dot={false} connectNulls />
          </>}
          {active.orders && <>
